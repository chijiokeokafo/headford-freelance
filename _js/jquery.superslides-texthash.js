...
*
* Modified to support text hashes by Jon Gacnik */
...
(function() {
  ...
  Superslides = function(el, options) {
    ...
    getTextHash = function(index){
    	return $children.eq(index).data('href');
    };
    getSlideTitle = function(index){
    	return $children.eq(index).data('name');
    };
    setupNextPrev = function() {
      return $("." + _this.options.classes.nav + " a").each(function() {
      	if ($(this).hasClass('next')) {
          return this.hash = _this.options.texthash ? getTextHash(that.next) : that.next;
        } else {
          return this.hash = _this.options.texthash ? getTextHash(that.prev) : that.prev;
        }
      });
    };
    addPaginationItem = function(i) {
      ...
      if(_this.options.texthash){
      	index = i;
      	name = getSlideTitle(i);
      	i = getTextHash(i);
      }
      return $pagination.append($("<a>", {
      	html: '<span class="project-title">'+name+'</span>', 
        href: "#" + i,
        "style": 'left:'+(index*64)+'px',
        "class": _this.current === $pagination.children().length ? "current" : void 0
      })).append('<span class="dot" style="left:'+((index*64)+40)+'px">&#x2022;</span>');
    };
    ...
    parseHash = function(hash) {
      ...
      hash = hash.replace(/^#/, '');
      if (hash) {
      	return _this.options.texthash ? +getPosByHash(hash) : +hash;
      }
    };
    ...
    animator = function(direction, callback) {
      ...
      $pagination.children().removeClass('current').eq((upcoming_slide*2)).addClass('current');
      return $control.animate({
        ...
        if (init) {
          ...
          $container.fadeIn('slow', function(){
          	$('.slides-container>li:first-child .reveal').css('opacity',1)
          });
          // $container.show();
    ...
    this.animate = function(direction, callback) {
      ...
      $container.trigger('begintransition.slides')
